...
class CalBankSummaryDetail extends StatefulWidget {
  final date_requested;
  final username;
  const CalBankSummaryDetail({Key? key, this.date_requested,required this.username})
      : super(key: key);

  @override
  _CalBankSummaryDetailState createState() =>
      _CalBankSummaryDetailState(date_requested: this.date_requested,username:this.username);
}

class _CalBankSummaryDetailState extends State<CalBankSummaryDetail> {
  final date_requested;
  final username;
  _CalBankSummaryDetailState({required this.date_requested,required this.username});

  final storage = GetStorage();
  bool hasToken = false;
  late String uToken = "";
  late List allCalRequests = [];
  bool isLoading = true;
  late var items;
  late List amounts = [];
  ...
  Future<void>fetchAllCalBankRequests() async {
    ...
    if (response.statusCode == 200) {
      ...
      allCalRequests = json.decode(jsonData);
      for (var i in allCalRequests) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Calbank summary for $date_requested"),
      ),
      body: SafeArea(
          child: isLoading
              ? const LoadingUi()
