...
class EcoBankSummaryDetail extends StatefulWidget {
  ...
  final username;
  const EcoBankSummaryDetail({Key? key, this.date_requested,required this.username})
      : super(key: key);

  @override
  _EcoBankSummaryDetailState createState() =>
      _EcoBankSummaryDetailState(date_requested: this.date_requested,username:this.username);
}

class _EcoBankSummaryDetailState extends State<EcoBankSummaryDetail> {
  final date_requested;
  final username;
  _EcoBankSummaryDetailState({required this.date_requested,required this.username});

  final storage = GetStorage();
  bool hasToken = false;
  late String uToken = "";
  late List allEcoRequests = [];
  bool isLoading = true;
  late var items;
  late List amounts = [];
  ...
  Future<void>fetchAllEcobankRequests() async {
    ...
    if (response.statusCode == 200) {
      ...
      allEcoRequests = json.decode(jsonData);
      for (var i in allEcoRequests) {
